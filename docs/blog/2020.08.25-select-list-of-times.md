---
title: Select List of Times
description: This is an HTML select list of times.
keywords: html code snippet, select list of times, select list, bash, linux, time selector
...
```bash
for h in {1..12}; do for m in {0..5}; do echo "<option value=\"$h:$m"0" AM\">$h:$m"0 AM"</option>"; done; done
for h in {1..12}; do for m in {0..5}; do echo "<option value=\"$h:$m"0" PM\">$h:$m"0 PM"</option>"; done; done
```

For the 12-hour clock, the 12 AM and PM times show up in the wrong place. However, my objective was to get them to show up, so that I did not have 
to type them manually. Moving already typed lines of code is easier than having to type them out.
...
This code is to be used as a pre-built snippet. I couldn't find one online, so I decided to create one in case I needed it in the future. 
